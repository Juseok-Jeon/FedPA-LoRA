...
class GLUETrainer(GeneralTorchTrainer):
    ...
    def _hook_on_batch_forward(self, ctx):
        ...
        if getattr(ctx.cfg.lora, "regularization", False) and hasattr(ctx, "global_lora_ref"):
            reg = 0.0
            ref = ctx.global_lora_ref
            reg_type = str(ctx.cfg.lora.reg).lower()
            ...
            for name, module in ctx.model.named_modules():
                if hasattr(module, "lora_B") and hasattr(module, "lora_A"):
                    for adapter in module.lora_A.keys():
                        ...
                        B_ref_key = next(
                            (k for k in ref.keys() if k.endswith(B_key) or B_key.endswith(k)),
                            None
                        )
                        A_ref_key = next(
                            (k for k in ref.keys() if k.endswith(A_key) or A_key.endswith(k)),
                            None
                        )
                        ...
                        if B_ref_key is not None and A_ref_key is not None:
                            ...
                            Bg = ref[B_ref_key].to(B.device).float()
                            Ag = ref[A_ref_key].to(A.device).float()
                            
                            # Regularization of BA
                            if reg_type == "product":
                                E = B @ A - Bg @ Ag
                                reg = reg + E.pow(2).sum()
                            # Regularization of B
                                ...
                                E_B = B - Bg
                                reg = reg + E_B.pow(2).sum()
                            # Regularization of A
                            elif reg_type == "a":
                                E_A = A - Ag
                                reg = reg + E_A.pow(2).sum()
                            # Regularization of B and A
                            elif reg_type == "both":
                                E_B = B - Bg
                                E_A = A - Ag
                                reg = (
            ...
            # Eq. 3
            loss = loss + 0.5 * float(ctx.cfg.lora.lam) * reg
    ...
    def _hook_on_fit_end(self, ctx):
        ...
        glue_metric = load('glue', ctx.cfg.data.type.split('@')[0], trust_remote_code=True, experiment_id=f"{os.getpid()}_{ctx.cur_split}_{id(ctx)}")
