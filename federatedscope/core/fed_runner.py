...
class StandaloneRunner(BaseRunner):
    def _set_up(self):
        ...
        if self.cfg.federate.share_local_model:
            if getattr(self.cfg.lora, "regularization", False):
                self._shared_client_model = copy.deepcopy(self.server.model)
            elif self.cfg.federate.online_aggr:
        ...
        for client_id in range(1, self.cfg.federate.client_num + 1):
            if getattr(self.cfg.lora, "regularization", False) and not self.cfg.federate.share_local_model:
                client_model = copy.deepcopy(self.server.model)
            else:
