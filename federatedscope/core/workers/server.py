...
class Server(BaseServer):
    ...
    def _perform_federated_aggregation(self):
        ...
        for model_idx in range(self.model_num):
            ...
            raw_msg_list = None
            if (
                self._cfg.lora.rotate
                and self.state > 0
                and self._cfg.lora.method == "shareAB"
            ):
                raw_msg_list = []
                clean_msg_list = []
                for size, state in msg_list:
                    raw_state = {}
                    clean_state = {}
                    for k, v in state.items():
                        if k.startswith("__raw__."):
                            raw_state[k[len("__raw__."):]] = v
                        else:
                            clean_state[k] = v
                    raw_msg_list.append((size, raw_state))
                    clean_msg_list.append((size, clean_state))
                msg_list = clean_msg_list
            ...
            if self.state == 1:
                state0 = msg_list[0][1]
                num_tensors = len(state0)
                total_elements = sum(v.numel() for v in state0.values())
                lora_tensors = {
                    k: v for k, v in state0.items()
                ...
                lora_elements = sum(
                    v.numel() for v in lora_tensors.values()
                )
                classifier_elements = sum(
                    v.numel() for v in classifier_tensors.values()
                )
                print("\n[Aggregation Info]")
                print("Number of tensors:", num_tensors)
                print("Total elements:", total_elements)
                print("Number of LoRA tensors:", len(lora_tensors))
                print("LoRA elements:", lora_elements)
                print("Number of classifier tensors:", len(classifier_tensors))
                print("Classifier elements:", classifier_elements)

            result = aggregator.aggregate(agg_info)

            # Sec. 4.3 Product-Space Aggregation
            svd_mode = str(
                getattr(self._cfg.lora, "svd", "none")
            ).lower()
            use_svd = (
                svd_mode != "none"
                and self._cfg.lora.method == "shareAB"
            ...
            if use_svd:
                original_msg_list = copy.deepcopy(msg_list)

            if use_svd:
                global_result = copy.deepcopy(result)
                reference_result = copy.deepcopy(result)

                client_states = [x[1] for x in original_msg_list]
                client_sizes = torch.tensor(
                    [float(x[0]) for x in original_msg_list],
                    dtype=torch.float32,
                )
                client_weights = client_sizes / client_sizes.sum()

                global_B_keys = sorted([
                    k for k in client_states[0].keys()
                    if "lora_B" in k and k.endswith("weight")
                ])

                reference_rank = max(
                    0,
                    int(getattr(self._cfg.lora, "reference_rank", 0)),
                )
                oversampling_p = max(
                    0,
                    int(getattr(self._cfg.lora, "oversampling_p", 2))
                )
 
                for B_key in global_B_keys:
                    A_key = B_key.replace("lora_B", "lora_A")
                    if A_key not in client_states[0]:
                        continue

                    B_template = client_states[0][B_key]
                    A_template = client_states[0][A_key]
                    device = B_template.device
                    dtype_B = B_template.dtype
                    dtype_A = A_template.dtype

                    global_rank = B_template.shape[1]  # R_g
                    
                    # Dense Product Aggregation and SVD
                    if svd_mode == "full":
                        delta_w_ideal = torch.zeros(
                            B_template.shape[0],
                            A_template.shape[1],
                            device=device,
                            dtype=torch.float32,
                        ...
                        for i, client_state in enumerate(client_states):
                            client_weight = client_weights[i].to(device)
                            B_i = client_state[B_key].detach().to(
                                device=device,
                                dtype=torch.float32,
                            )
                            A_i = client_state[A_key].detach().to(
                                device=device,
                                dtype=torch.float32,
                            )
                            delta_w_ideal += client_weight * (B_i @ A_i)

                        U_global, singular_values, Vh_global = torch.linalg.svd(
                            delta_w_ideal,
                            full_matrices=False,
                        )
                        effective_rank = min(
                        ...
                        U_global = U_global[:, :effective_rank]
                        Sigma_global = singular_values[:effective_rank]
                        Vh_global = Vh_global[:effective_rank, :]
                        ...
                        A_cat_list = []
                        for i, client_state in enumerate(client_states):
                            sqrt_weight_i = torch.sqrt(
                                client_weights[i].to(device)
                            )
                            B_i = client_state[B_key].detach().to(
                                device=device,
                                dtype=torch.float32,
                            ...
                            B_cat_list.append(sqrt_weight_i * B_i)
                            A_cat_list.append(sqrt_weight_i * A_i)
                        B_cat = torch.cat(B_cat_list, dim=1)
                        A_cat = torch.cat(A_cat_list, dim=0)

                        # Reduced QR and Core SVD
                        if svd_mode == "reduced":
                            # Eq. 5
                            Q_B, R_B = torch.linalg.qr(
                                B_cat,
                                mode="reduced",
                            )
                            ...
                            H = R_B @ R_A.T
                            U_H, singular_values, Vh_H = torch.linalg.svd(
                                H,
                                full_matrices=False,
                            )
                            effective_rank = min(
                            ...
                            U_Rg = U_H[:, :effective_rank]
                            Sigma_global = singular_values[:effective_rank]
                            Vh_Rg = Vh_H[:effective_rank, :]
                            U_global = Q_B @ U_Rg
                            Vh_global = Vh_Rg @ Q_A.T

                        # Appendix G.2 Randomized Server-Side Reconstruction
                        elif svd_mode == "random":
                            total_rank = B_cat.shape[1]

                            sketch_dim = min(
                                global_rank + oversampling_p,
                                total_rank,
                                B_cat.shape[0],
                                A_cat.shape[1],
                            )  
                            ...
                            U_projected, singular_values, Vh_global = (
                                torch.linalg.svd(
                                    projected_matrix,
                                    full_matrices=False,
                                )
                            ...
                            U_global = (
                                Q_random
                                @ U_projected[:, :effective_rank]
                            )
                            Sigma_global = singular_values[:effective_rank]
                            Vh_global = Vh_global[:effective_rank, :]

                    # Eq. 6
                    B_g = U_global * Sigma_global.unsqueeze(0)
                    A_g = Vh_global

                    # Construct the rank-R_i global reference
                    R_i = (
                        effective_rank
                        if reference_rank == 0
                        else min(reference_rank, effective_rank)
                    )
                    B_gi = U_global[:, :R_i] * Sigma_global[:R_i].unsqueeze(0)
                    A_gi = Vh_global[:R_i, :]
                    reference_result[B_key] = B_gi.to(dtype=dtype_B)
                    reference_result[A_key] = A_gi.to(dtype=dtype_A)

                    if effective_rank < global_rank:
                        B_g_padded = torch.zeros(
                            B_template.shape[0],
                            global_rank,
                            device=device,
                            dtype=torch.float32,
                        )
                        A_g_padded = torch.zeros(
                            global_rank,
                            A_template.shape[1],
                            device=device,
                            dtype=torch.float32,
                        )
                        B_g_padded[:, :effective_rank] = B_g
                        A_g_padded[:effective_rank, :] = A_g
                        torch.nn.init.kaiming_uniform_(
                            A_g_padded[effective_rank:, :],
                            a=math.sqrt(5),
                        )
                        B_g = B_g_padded
                        A_g = A_g_padded

                    global_result[B_key] = B_g.to(dtype=dtype_B)
                    global_result[A_key] = A_g.to(dtype=dtype_A)

                result = global_result
                self.global_lora_ref = reference_result
            ...
            if self._cfg.lora.merge and ((self.state + 1) % int(self._cfg.lora.merge_step) == 0):
                with torch.no_grad():
                    for name, module in model.named_modules():
                        ...
                        for adapter_name in module.lora_A.keys():
                            ...
                            delta_w = (B.float() @ A.float())
                            if getattr(module, "fan_in_fan_out", False):
                                delta_w = delta_w.T
                            delta_w = delta_w * float(scaling)

                            module.base_layer.weight.data.add_(
                                delta_w.to(
                            ...
                            torch.nn.init.kaiming_uniform_(A_layer.weight, a=math.sqrt(5))
                            torch.nn.init.zeros_(B_layer.weight)
    ...
    def broadcast_model_para(self,
        ...
        if (
            msg_type in ['model_para', 'evaluate']
            and getattr(self._cfg.lora, "regularization", False)
            and self.global_lora_ref is not None
        ):
            model_para = copy.deepcopy(self.global_lora_ref)
