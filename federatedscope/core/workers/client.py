...
class Client(BaseClient):
    ...
    def __init__(self,
        ...
        if getattr(self._cfg.lora, "regularization", False):
            self._local_model_state = {
                k: v.detach().cpu().clone()
                for k, v in self.trainer.ctx.model.state_dict().items()
            }
    ...
    def callback_funcs_for_model_para(self, message: Message):
        ...
        if 'ss' in message.msg_type:
            ...
            if not self._cfg.lora.regularization:
                self.trainer.update(content,
                                    strict=self._cfg.federate.share_local_model)
            else:
                self.trainer.ctx.global_lora_ref = copy.deepcopy(content)

                # Sec. 4.1 Local Factor Preservation
                model_state = self.trainer.ctx.model.state_dict()
                with torch.no_grad():
                    for k, v in self._local_model_state.items():
                        if k in model_state:
                            model_state[k].copy_(
                                v.to(model_state[k].device, dtype=model_state[k].dtype)
                            )
            ...
            if self.is_unseen_client or skip_train_isolated_or_global_mode:
                ...
                if self._cfg.lora.rolora is True:
                    ...
                    for name, param in self.trainer.ctx.model.named_parameters():
                        if "lora_A" in name or "lora_B" in name:
                            param.requires_grad = remove_lora not in name
                ...
                raw_model_para_all = None
                if (
                    self._cfg.lora.rotate
                    and self.state > 0
                    and self._cfg.lora.method == "shareAB"
                ):
                    raw_model_para_all = copy.deepcopy(model_para_all)
                ...
                if raw_model_para_all is not None:
                    for k, v in raw_model_para_all.items():
                        if (
                            ("lora_A" in k or "lora_B" in k)
                            and k.endswith("weight")
                        ):
                            model_para_all[f"__raw__.{k}"] = v.detach().clone()
    ...
    def callback_funcs_for_evaluate(self, message: Message):
        ...
        train_model = self.trainer.ctx.model
        eval_model = copy.deepcopy(train_model)
        if message.content is not None:
            # Evaluate using the rank-R_i global reference
            reference_rank = int(
                getattr(self._cfg.lora, "reference_rank", 0)
            )
            if (
                getattr(self._cfg.lora, "regularization", False)
                and reference_rank > 0
            ):
                with torch.no_grad():
                    for name, module in eval_model.named_modules():
                        ...
                        for adapter_name in module.lora_A.keys():
                            B_key = f"{name}.lora_B.{adapter_name}.weight"
                            A_key = f"{name}.lora_A.{adapter_name}.weight"
                            if B_key not in message.content or A_key not in message.content:
                                continue
                            B = message.content[B_key].to(
                                device=module.base_layer.weight.device,
                                dtype=torch.float32
                            )
                            A = message.content[A_key].to(
                                device=module.base_layer.weight.device,
                                dtype=torch.float32
                            )
                            delta_w = B @ A
                            ...
                            module.base_layer.weight.data.add_(
                                delta_w.to(dtype=module.base_layer.weight.dtype)
                            )
                ...
                for k, v in message.content.items():
                    if k in eval_state:
                        eval_state[k] = v.to(
                            eval_state[k].device,
                            dtype=eval_state[k].dtype
                        )
            
            if getattr(self._cfg.lora, "regularization", False):
                local_state = getattr(self, "_local_model_state", {})
            else:
                local_state = train_model.state_dict()
            for local_key in self._cfg.personalization.local_param:
                for k in eval_state:
                    if local_key in k and k in local_state:
                        eval_state[k] = local_state[k].to(
                            eval_state[k].device,
                            dtype=eval_state[k].dtype
                        )
            eval_model.load_state_dict(eval_state, strict=False)
        
        self.trainer.ctx.model = eval_model
        ...
        if self.early_stopper.early_stopped and self._cfg.federate.method in [
            ...
            raw_results = formatted_eval_res['Results_raw']
            best_key = self._cfg.eval.best_res_update_round_wise_key
            if best_key in raw_results:
        ...
        self.trainer.ctx.model = train_model
        del eval_model
